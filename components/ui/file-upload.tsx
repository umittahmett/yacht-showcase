...
export const FileUpload = ({ onChange, value = [] }: FileUploadProps) => {
  ...
  return (
    <div className="w-full rounded-md border bg-white border-border" {...getRootProps()}>
      ...
      >
        <div onClick={handleClick} className="z-20 cursor-pointer absolute inset-0"></div>
        <input
          ref={fileInputRef}
          multiple
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="relative w-full mt-10 grid grid-cols-2 gap-6">
            {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  key={"file" + idx}
                  ...
                  className={cn(
                    "relative z-50 bg-white dark:bg-neutral-900 flex flex-col items-start justify-start md:h-24 p-4 mt-4 w-full mx-auto rounded-md",
                    "shadow-sm"
                  )}
                >
            ...
            {!files.length && (
              <motion.div
                ...
                className="absolute opacity-0 border border-dashed cursor-pointer border-sky-400 inset-0 z-10 bg-transparent flex items-center justify-center h-32 mt-4 w-full max-w-[8rem] mx-auto rounded-md"
              ></motion.div>
            )}
          </div>
        </div>
